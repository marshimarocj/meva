...
ENC = model.conv_transformer.ENC
...
def load_and_fill_model_cfg(model_cfg_file, path_cfg):
  model_cfg = model.conv_transformer.ModelConfig()
...
if __name__ == '__main__':
  ...
  if opts.is_train:
    ...
    trntst = model.conv_transformer.TrnTst(model_cfg, path_cfg, m)

    trn_reader = model.conv_transformer.Reader(
      path_cfg.label2lid_file, path_cfg.trn_dirs, model_cfg.subcfgs[ENC].num_step, model_cfg.shift, shuffle=True)
    val_reader = model.conv_transformer.ValReader(
