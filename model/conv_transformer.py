...
class Model(framework.model.module.AbstractModel):
  ...
  def _set_submods(self):
    return {
      ...
      ENC: encoder.conv_transformer.Encoder(self._config.subcfgs[ENC]),
    }
  ...
  def get_out_ops_in_mode(self, in_ops, mode):
    ...
    out_ops = self.submods[ENC].get_out_ops_in_mode({
      self.submods[ENC].InKey.FT: out_ops[self.submods[PCA].OutKey.EMBED],
      self.submods[ENC].InKey.MASK: in_ops[self.InKey.MASK],
      self.submods[ENC].InKey.IS_TRN: in_ops[self.InKey.IS_TRN],
    }, mode)

    outputs = out_ops[self.submods[ENC].OutKey.OUTPUT] # (None, num_step, dim_hidden)
